...
@bot.slash_command(name="say", description="Repeats the message sent by the user.")
...
@bot.slash_command(
    name="register", description="Say this to register for Timezone and title."
)
@discord.option(
    name="gmt",
...
async def register(ctx, gmt: str = None, title: str = None):
...
@bot.slash_command(
    name="get_user_info", description="Displays profile information about the user."
)
@discord.option(
    name="user_id",
...
async def get_user_info(ctx, user_id: str = None):
...
@bot.slash_command(name="whoami", description="Displays profile information about you.")
...
@bot.slash_command(name="say_time", description="Displays the time in GMT.")
async def say_time(ctx):
    ...
    if result:
        time = datetime.datetime.now(
            datetime.timezone(datetime.timedelta(hours=result["gmt"]))
        )
        await ctx.respond(
            embed=discord.Embed(title="Time", description=f"{time}", color=pink)
        )
    else:
        await ctx.respond(
            embed=discord.Embed(
...
@bot.slash_command(name="say_embed", description="Repeats the message inside an embed.")
@discord.option(
    name="nachricht",
...
async def say_embed(ctx, nachricht: str = None):

    nachricht = nachricht or ""
    await ctx.respond(
        embed=discord.Embed(
            title=ctx.author.name + ": Says", description=nachricht, color=purple
        )
    )
...
@bot.slash_command(name="testembed", description="test embed.")
async def testembed(ctx):

    embed = discord.Embed(
        title="Test Embed", description="Das ist ein Test Embed.", color=0xC586C0
    )
    await ctx.respond(embed=embed)


@bot.slash_command(name="stop", description="Stops the bot.")
async def stop(ctx):
...
@bot.slash_command(
    name="ping", description="Measures latency between the server and a given IP."
)
async def ping(ctx, ip: str = None, count: int = None):
    await ctx.defer()
...
async def palc(ctx, text: str = None, embed: bool = True):
    await ctx.defer()
    ...
    if embed == True:
        embed = discord.Embed(
            title="Palisch Cutiee Lyanc :3", description=result, color=dark_blue
        )
        await ctx.respond(embed=embed)
    else:
...
async def zar(
    ctx, lower_bound: int = None, upper_bound: int = None, embed: bool = True, call=None
):
    ...
    if call == "zar_":
        ...
        if embed == True:
            embed = discord.Embed(title="Zar", description=result, color=dark_blue)
            await ctx.respond(embed=embed)
        else:
        ...
        if embed == True:
            embed = discord.Embed(
                title="Random Number", description=result, color=dark_blue
            )
            await ctx.respond(embed=embed)
        else:
...
@bot.slash_command(name="zar", description="Generate a random number.")
async def zar_(
    ctx, lower_bound: int = None, upper_bound: int = None, embed: bool = True
):

    await zar(ctx, lower_bound, upper_bound, embed, call="zar_")


@bot.slash_command(name="random_number", description="Generate a random number.")
async def random_number(
    ctx, lower_bound: int = None, upper_bound: int = None, embed: bool = True
):

    await zar(ctx, lower_bound, upper_bound, embed)


@bot.slash_command(name="choose", description="Choose a random word from a given text.")
async def choose(
    ctx,
    text: str = None,
    embed: bool = True,
    ...
    if countrys == True:
        ...
        if embed == True:
            embed = discord.Embed(
                title="Choose a country", description=result, color=dark_blue
            )
            await ctx.respond(embed=embed)
        else:
        ...
        if embed == True:
            embed = discord.Embed(
                title="Choose a name", description=result, color=dark_blue
            )
            await ctx.respond(embed=embed)
        else:
        ...
        if embed == True:
            embed = discord.Embed(
                title="Choose a word", description=result, color=dark_blue
            )
            await ctx.respond(embed=embed)
...
@bot.slash_command(name="help", description="Provides information about the bot.")
async def help(ctx):
    ...
    embed.add_field(
        name="github", value="[Click here](https://github.com/FuffyPal)", inline=True
    )
